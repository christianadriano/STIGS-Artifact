...
def read_architecture_from_xml(filename) -> networkx.DiGraph:
    ...
    for child in root:
        ...
        vertex_label = child.attrib.get("label", None)  # Fetch the label; if it does not exist, assign None

        if vertex_id not in existing_nodes:
            existing_nodes.add(vertex_id)
            network_graph.add_node(vertex_id, type='service', label=vertex_label)

        if len(list(child)) > 0:
            for adjacent_vertex in child:
                adjacent_vertex_id = adjacent_vertex.attrib["vertex"]
                adjacent_vertex_label = adjacent_vertex.attrib.get("label", None)  # Do the same for the adjacent vertices

                if adjacent_vertex_id not in existing_nodes:
                    existing_nodes.add(adjacent_vertex_id)
                    network_graph.add_node(adjacent_vertex_id, type='service', label=adjacent_vertex_label)
...
def plot_graph(plottable_graph: networkx.Graph, has_edge_labels: bool = False, integer_labels: bool = False,
               layout=networkx.layout.spring_layout, color_map=None, relabeled=False, node_size=500):
    matplotlib.pyplot.figure(figsize=(15, 10))  # Create a new figure with a custom size (in inches)
    ...
    if integer_labels:
        plottable_graph = networkx.convert_node_labels_to_integers(plottable_graph)
    pos = layout(plottable_graph)
    networkx.draw_networkx(plottable_graph, pos, node_color=color_map, node_size=node_size, with_labels=False)

    # Combine node ID and label into one label
    node_labels = {node: f"{node}: {plottable_graph.nodes[node].get('label', '')}" for node in plottable_graph.nodes()}
    networkx.draw_networkx_labels(plottable_graph, pos, labels=node_labels)
...
def main():
    graph = read_architecture_from_xml("data/Architecture-Diagram_Interference_pap.xml")
    apply_deployment(graph, 'data/deployment_Interference_pap.yaml')

    plot_graph(graph)

    # print(get_host(graph, "user-db"))
    # move_service_to_host(graph, "user-db", "worker1")
    #print(get_host(graph, "orders"))
    #
